...
namespace Microsoft.AspNetCore.OData.E2E.Tests.DollarApply
{
    public class DollarApplyTests : WebApiTestBase<DollarApplyTests>
    {
        ...
        protected static void UpdateConfigureServices(IServiceCollection services)
        {
            ...
            services.ConfigureControllers(typeof(SalesController), typeof(EmployeesController));
        ...
        public async Task TestComputeNestedStringPropertyLengthThenGroupByNestedPropertyAndAggregatePrimitiveAndComputedPropertyAsync(string queryUrl)
        {
            ...
            var resultAt1 = Assert.IsType<JObject>(result[1]);
            var resultAt2 = Assert.IsType<JObject>(result[2]);
            Assert.Equal(5, resultAt0.Value<int>("MaxProductNameLength"));
            Assert.Equal(21, resultAt0.Value<int>("Total"));
            Assert.Equal("Paper", Assert.IsType<JObject>(resultAt0.GetValue("Product")).Value<string>("Name"));
            Assert.Equal(5, resultAt1.Value<int>("MaxProductNameLength"));
            Assert.Equal(8, resultAt1.Value<int>("Total"));
            Assert.Equal("Sugar", Assert.IsType<JObject>(resultAt1.GetValue("Product")).Value<string>("Name"));
            Assert.Equal(6, resultAt2.Value<int>("MaxProductNameLength"));
            Assert.Equal(7, resultAt2.Value<int>("Total"));
            Assert.Equal("Coffee", Assert.IsType<JObject>(resultAt2.GetValue("Product")).Value<string>("Name"));
        }
